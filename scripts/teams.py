...
class TeamRating:
    ...
    def update_q(self, players_release):
        ...
        """
        top_h = self.data.iloc[:constants.TOP_TEAMS_FOR_Q_CALCULATION]
        ...
        rb_raws = players_release.data[players_release.data['base_team_id'].isin(top_h_ids)].groupby(
            ...
            lambda x: calc_tech_rating(x.values) if len(
                x.values) >= constants.PLAYERS_IN_TEAM_FOR_Q_CALCULATION else None).dropna()
    ...
    def calc_c(self):
        ...
        ratings[::-1].sort()
        return constants.MAX_BONUS / ratings[:constants.TEAMS_COUNT_FOR_BP].dot(
            2. ** np.arange(0, -constants.TEAMS_COUNT_FOR_BP, -1))
    ...
    def update_ratings_for_changed_teams(self, changed_teams) -> List[Tuple[int, int]]:
        ...
        self.data.loc[existing_teams, 'rating'] = np.maximum(
            ...
            self.data.loc[existing_teams, "trb"] * constants.NEW_TEAMS_LOWERING_COEFFICIENT)
        res = []
    ...
    def add_new_teams(self, tournament: Tournament, player_rating: PlayerRating):
        ...
        new_teams['rating'] = new_teams['trb'] * constants.NEW_TEAMS_LOWERING_COEFFICIENT
